...
//Septima funcion (pre entrega 7 y pre entrega 10 (modificación swal))
...
function crearPrestamoDesdeFormulario() {
  ...
  if (isNaN(monto) || monto < 100000 || monto > 10000000 || isNaN(plazoMeses) || plazoMeses <= 0) {
    Swal.fire({
      ...
      text: "Revisá el monto (entre 100.000 y 10.000.000) y el plazo."
    });
    return;
  }
  ...
  Swal.fire({
    ...
    text: "Préstamo de tipo " + tipo + " creado con éxito. Cuota mensual: $" + nuevoPrestamo.calcularCuota().toFixed(2)
  });
...
//onceaba función (pre-entrega-10)
async function obtenerCotizaciones() {
  contenedorCotizaciones.innerHTML = "<p>Cargando cotizaciones...</p>";
  ...
  try {
    ...
    let tarjetas = datos.map(cotizacion => `
    ...
    contenedorCotizaciones.innerHTML = tarjetas.join("");
    
  } catch (error) {
    contenedorCotizaciones.innerHTML = "<p>No se pudieron cargar las cotizaciones.</p>";
    Swal.fire({
      icon: "error",
      title: "Error de conexión",
      text: "No pudimos obtener la cotización del dólar. Intentá nuevamente más tarde."
    });
  } finally {
    console.log("Petición de cotizaciones finalizada.");
  }
}
...
//try-catch-finally (pre entrega 9)
if (datosGuardados !== null) {
  try {
    ...
    console.error("Error al leer los datos guardados:", error.message);
    listaPrestamos = [prestamo1, prestamo2, prestamo3, prestamo4, prestamo5, prestamo6];
  } finally {
    console.log("Proceso de carga de préstamos finalizado.");
  }
} else {
  listaPrestamos = [prestamo1, prestamo2, prestamo3, prestamo4, prestamo5, prestamo6];
}
...
//(pre entrega 9)
let datosCuriosos = [
  "¿Sabías que el interés simple no capitaliza, a diferencia del interés compuesto?",
  "¿Sabías que cuanto más corto el plazo, menor el interés total que vas a pagar?",
  "¿Sabías que comparar tasas entre tipos de préstamo puede ahorrarte dinero?",
  "¿Sabías que un préstamo hipotecario suele tener plazos mucho más largos que uno personal?"
];

setTimeout(function() {
  let indiceAleatorio = Math.floor(Math.random() * datosCuriosos.length);
  let mensajeElegido = datosCuriosos[indiceAleatorio];
  mensajeFeedback.textContent = mensajeElegido;
  mensajeFeedback.style.color = "blue";
}, 4000);
//-------------------
